...
def _quaternion_to_euler_xyz(x, y, z, w):
...
def _euler_xyz_to_quaternion(roll, pitch, yaw):
...
class AgxArmRosNode(Node):
    ...
    def _declare_parameters(self):
        ...
        self.declare_parameter("can_index", -1)
    ...
    def _load_parameters(self):
        ...
        self.can_index = self.get_parameter("can_index").value
    ...
    def _log_parameters(self):
        ...
        self.get_logger().info(f"can_interface: {self.can_interface}")
        self.get_logger().info(f"can_index: {self.can_index}")
    ...
    def _init_agx_arm(self):
        ...
        if self.can_index >= 0:
            config_kwargs["index"] = self.can_index
        ...
        if not self.fw_version and self.firmware_profile != PiperFW.DEFAULT:
            ...
            if self.can_index >= 0:
                reconnect_kwargs["index"] = self.can_index
            config = create_agx_arm_config(**reconnect_kwargs)
            self.agx_arm = AgxArmFactory.create_arm(config)
            self.agx_arm.connect()
    ...
    def _create_pose_cmd(self, pose: Pose) -> list:
        ...
        euler_angles = _quaternion_to_euler_xyz(*quaternion)
    ...
    def _publish_pose(self):
        ...
        quaternion = _euler_xyz_to_quaternion(roll, pitch, yaw)
